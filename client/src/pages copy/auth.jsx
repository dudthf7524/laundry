...
const Auth = () => {
    ...
    const authUpdate = () => {
        console.log(selectedAuth.auth_code)
                ...
                <ul className="divide-y divide-gray-200">
                    {filteredUserLists ? (
                        filteredUserLists?.map((userList, index) => (
                            <li
                                key={index}
                                className={`p-4 hover:bg-gray-50 cursor-pointer ${selected.user_code === userList.user_code ? 'bg-blue-50' : ''
                                    }`}
                                onClick={() => handleEmployeeSelect(userList)}
                            >
                                <div className="flex flex-col md:flex-row md:items-center md:justify-between">
